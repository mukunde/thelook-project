...
- **Operational simplicity**: this is a single-operator portfolio project. A cap strategy that requires daily attention or third-party tooling is not sustainable.
...
- **Option C — Upgrade to PayG with a four-layer defense:** Compartment Quotas (hard block), Budget canary at 0.01 EUR (early detection), MFA on the tenancy admin user (anti-compromise), and IaC-only resource creation (audit trail).
...
- The four-layer defense produces a portfolio-relevant artefact: a small but realistic FinOps governance setup, expressible in Terraform and reviewable as code.
