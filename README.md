...
**Status:** Phase 1 (Infrastructure & Governance) closed: Snowflake RBAC, OCI VM, Terraform Cloud workflows, 13 ADRs. See the [Phase 1 closure report](docs/infrastructure-and-governance-phase-report.md). Phase 2 (data engineering) in progress: dlt ingestion, dbt Finance marts, the Cube Cloud semantic layer, Metabase self-service BI, and the KTX context layer for AI agents (MCP) are all live. The same canonical metrics are consumed by analysts (Cube Explore), end-users (Metabase), and AI agents (Claude via KTX MCP), cross-validated to the cent, with a [Loom walk-through](https://www.loom.com/share/3dade46e6d044e0891a13e927e634fc8) of the agent demo. Blog post is next.
...
| Cube Cloud semantic layer (Finance) | `https://thelook.cubecloud.dev/` (auth-protected) | 📡 ![Live](https://img.shields.io/badge/Live-brightgreen?style=flat-square) auto-deployed on merge to main, [Loom walk-through](https://www.loom.com/share/e25f23ff562f4604a4408b2c64aa416d) (modeling, view-only pattern, generated SQL, metric unicity) + screenshots below |
...
| AI agent demo (Cube semantic layer via KTX/MCP) | [Loom walk-through](https://www.loom.com/share/3dade46e6d044e0891a13e927e634fc8) | 📡 ![Live](https://img.shields.io/badge/Live-brightgreen?style=flat-square) Claude querying the `finance` view through KTX MCP: metric unicity, the analytical-horizon guard, and the financial-return-rate rationale |
